...
export default function CheckOut() {
  ...
  useEffect(() => {
    if (success === 'completed') {
      const updatedCart = cartItems.product.map(p => {
        const cartItem = cartItems.cart.find(c => p._id === c.product_id);
        if (cartItem) {
          return {
            product_id: p._id,
            title: p.title,
            ...
            total_price: p.price.sales_price * cartItem.quantity
          };
        }
        return null;
            ...
            <tr>
              ...
              <td>
                <input
                  ...
                  id="success_url"
                  name="success_url"
                  value={`http://localhost:3000/users/${slug}/esewacheckout`}
