  ...
  const { values, error, handleChange, handleSubmit } = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        setIsLoading(false); // Hide loader after receiving response
      }
    }
  });
                ...
                <div className="grid gap-y-4">
                  <div>
                    ...
                    <div className="relative">
                      <input type="email" id="email" name="email" onChange={handleChange} value={values.email} className="py-3 px-4 block w-full border-2 border-gray-200 rounded-md text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm" required aria-describedby="email-error" />
                    </div>
